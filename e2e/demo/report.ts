...
export interface DemoReport {
  ...
  phases: PhaseResult[];
  acliVersion: string;
  reviewTest: ReviewTestResult | null;
}
...
<body>
  ...
  <div class="container">
    ...
    ${report.reviewTest ? `
    <section class="phase">
      <h2>Review Preservation Test</h2>
      <div class="status ${report.reviewTest.passed ? 'status-success' : 'status-error'}">
        ${report.reviewTest.passed ? 'PASSED' : 'FAILED'}
      </div>
      <table>
        <tr><td><strong>Card ID</strong></td><td>${report.reviewTest.cardId}</td></tr>
        <tr><td><strong>Reviews before re-sync</strong></td><td>${report.reviewTest.reviewsBefore}</td></tr>
        <tr><td><strong>Reviews after re-sync</strong></td><td>${report.reviewTest.reviewsAfter}</td></tr>
        <tr><td><strong>Details</strong></td><td>${escapeHtml(report.reviewTest.details)}</td></tr>
      </table>
    </section>
    ` : ''}
