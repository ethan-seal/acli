...
use crate::types::{Card, CardId, CardInfo, DeckConfig};
...
pub trait AnkiCollection {
    ...
    fn update_card(&mut self, card_id: CardId, card: &Card) -> std::result::Result<(), Self::Error>;
    fn delete_card(&mut self, card_id: CardId) -> std::result::Result<(), Self::Error>;
    fn get_cards_in_deck(&mut self, deck: &DeckConfig) -> std::result::Result<Vec<CardInfo>, Self::Error>;
    ...
    fn rename_deck(&mut self, old_name: &str, new_name: &str) -> std::result::Result<(), Self::Error>;
    fn get_all_deck_names(&mut self) -> std::result::Result<Vec<String>, Self::Error>;
    fn move_cards_to_deck(&mut self, card_ids: &[CardId], target_deck: &DeckConfig) -> std::result::Result<(), Self::Error>;
}
...
impl DefaultAnkiCollection {
    ...
    pub fn new() -> Result<Self> {
        ...
        let collection = CollectionBuilder::default().build()
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to create collection: {}", e)))?;
        Ok(Self { collection })
    }
    ...
    pub fn open_collection_path<P: AsRef<std::path::Path>>(path: P) -> Result<Self> {
        use anki::collection::CollectionBuilder;
        let collection = CollectionBuilder::new(path.as_ref()).build()
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to open collection: {}", e)))?;
        Ok(Self { collection })
    }
}
...
impl AnkiCollection for DefaultAnkiCollection {
    ...
    fn move_cards_to_deck(&mut self, _card_ids: &[CardId], _target_deck: &DeckConfig) -> Result<()> {
        Err(AnkiWrapperError::AnkiError(
            "real Anki backend not compiled (missing 'real-anki' feature)".to_string(),
        ))
    }
...
impl AnkiCollection for DefaultAnkiCollection {
    ...
    fn add_card(&mut self, deck: &DeckConfig, card: &Card) -> Result<()> {
        ...
        let deck_obj = self.collection
            .get_or_create_normal_deck(&deck.name)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get/create deck: {}", e)))?;
        ...
        let notetype = match card.card_type {
            crate::types::CardType::Basic => {
                ...
                notetypes
                    ...
                    .ok_or_else(|| AnkiWrapperError::AnkiError("Basic notetype not found".to_string()))?
                    .clone()
            }
            crate::types::CardType::BasicReversed => {
                ...
                notetypes
                    ...
                    .ok_or_else(|| AnkiWrapperError::AnkiError("Basic (and reversed card) notetype not found".to_string()))?
                    .clone()
            }
        };
    ...
    fn clear_deck(&mut self, deck: &DeckConfig) -> Result<()> {
        ...
        for card_id in &card_ids {
            if let Some(card) = self.collection.storage.get_card(*card_id)
        ...
        if !note_ids.is_empty() {
            self.collection
                ...
                .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to remove notes: {}", e)))?;
        }
    ...
    fn create_deck(&mut self, deck: &DeckConfig) -> Result<()> {
        ...
        let existing_deck_id = self.collection
            .get_deck_id(&deck.name)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to check deck existence: {}", e)))?;
    ...
    fn update_card(&mut self, card_id: CardId, card: &Card) -> Result<()> {
        ...
        let anki_card = self.collection.storage.get_card(anki::card::CardId(card_id.0))
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get card: {}", e)))?
            .ok_or_else(|| AnkiWrapperError::CardNotFound { id: card_id })?;

        // Get the note
        let mut note = self.collection.storage.get_note(anki_card.note_id())
        ...
        self.collection.update_note(&mut note)
    ...
    fn delete_card(&mut self, card_id: CardId) -> Result<()> {
        ...
        self.collection.remove_notes(&[anki_card.note_id()])
    ...
    fn get_cards_in_deck(&mut self, deck: &DeckConfig) -> Result<Vec<CardInfo>> {
        ...
        // Search for all cards in the deck
        let search_query = format!("deck:\"{}\"", deck.name);
        let card_ids = self.collection.search_cards(&search_query, SortMode::NoOrder)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to search cards: {}", e)))?;

        let mut result = Vec::new();
        for anki_card_id in card_ids {
            if let Some(anki_card) = self.collection.storage.get_card(anki_card_id)
                .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get card: {}", e)))? {

                // Get the note to access fields
                if let Some(note) = self.collection.storage.get_note(anki_card.note_id())
                    .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get note: {}", e)))? {

                    // Determine card type based on note type name
                    let notetype = self.collection.get_notetype(note.notetype_id)
                        .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get notetype: {}", e)))?
                        .ok_or_else(|| AnkiWrapperError::AnkiError("Notetype not found".to_string()))?;
    ...
    fn deck_exists(&mut self, deck: &DeckConfig) -> Result<bool> {
        let deck_id = self.collection.get_deck_id(&deck.name)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to check deck existence: {}", e)))?;
        Ok(deck_id.is_some())
    }
    ...
    fn rename_deck(&mut self, old_name: &str, new_name: &str) -> Result<()> {
        // Get the deck ID
        let deck_id = self.collection.get_deck_id(old_name)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get deck ID: {}", e)))?
            .ok_or_else(|| AnkiWrapperError::DeckNotFound {
        ...
        let deck = self.collection.get_deck(deck_id)
        ...
        self.collection.add_or_update_deck(&mut deck_mut)
    ...
    fn get_all_deck_names(&mut self) -> Result<Vec<String>> {
        let all_deck_names = self.collection.get_all_deck_names(false)
    ...
    fn move_cards_to_deck(&mut self, card_ids: &[CardId], target_deck: &DeckConfig) -> Result<()> {
        ...
        let deck_obj = self.collection.get_or_create_normal_deck(&target_deck.name)
            .map_err(|e| AnkiWrapperError::AnkiError(format!("Failed to get/create target deck: {}", e)))?;
        let deck_id = anki::decks::DeckId(deck_obj.id.0);

        // Convert CardId to anki::card::CardId
        let anki_card_ids: Vec<anki::card::CardId> = card_ids.iter()
            .map(|id| anki::card::CardId(id.0))
            .collect();

        // Move cards to the deck
        self.collection.set_deck(&anki_card_ids, deck_id)
